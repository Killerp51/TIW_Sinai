...
my_ctld.filename = "TIW_Sinai.csv"
...
-- **********************************************************************************************************************************************************************************************************************************************************************
   ...
   Color_Blue = {0,0,255}
   Color_Red = {255,0,0}
   Color_Green = {0,255,0} -- Logistics Zone
   Color_Gray = {128,128,128} -- Neutral
...
-- Zones (These are used for F10 drawings)
   ...
   KedemCapture = ZONE:New("Kedem")
   HatzerimCapture = ZONE:New("Hatzerim")
   NevatimCapture = ZONE:New("Nevatim")

   -- Zone Initialization (The actual monitoring script for capture)
   -- Blue
   ...
   HatzorCoalition = ZONE_CAPTURE_COALITION:New( HatzorCapture, coalition.side.RED):SetMarkZone( false ):Start() -- Guarded and Capturable
   KedemCoalition = ZONE_CAPTURE_COALITION:New( KedemCapture, coalition.side.RED):SetMarkZone( false )
   HatzerimCoalition = ZONE_CAPTURE_COALITION:New( HatzerimCapture, coalition.side.RED):SetMarkZone( false )
   NevatimCoalition = ZONE_CAPTURE_COALITION:New( NevatimCapture, coalition.side.RED):SetMarkZone ( false )


   -- Command Center Initialization

   BLUE_HQ = COMMANDCENTER:New( GROUP:FindByName( "BLUEHQ" ), "Israeli C2" )
   RED_HQ = COMMANDCENTER:New( GROUP:FindByName( "REDHQ" ), "Egyptian C2" )

   -- Functions

   --[[ function Capture2Blue()
      -- Draw second set of zones
      KedemCapture:DrawZone(-1, Color_Gray, 1, Color_Gray, .5, 1, true)
      HatzerimCapture:DrawZone(-1, Color_Gray, 1, Color_Gray, .5, 1, true)
      NevatimCapture:DrawZone(-1, Color_Gray, 1, Color_Gray, .5, 1, true)
...
--   
   ...
   -- Setup SHORAD
   local redSAMS = SET_GROUP:New():FilterPrefixes("Red SAM"):FilterCoalitions("red"):FilterStart()
   RedSHORAD = SHORAD:New("RedShorad", "Red SHORAD", redSAMS, 25000, 600, "red", true)
   RedSHORAD:SwitchHARMDefense(true)
   RedSHORAD:SetDefenseLimits(5, 95)
   RedSHORAD:SetDefenseRadius(1000)
   RedSHORAD:SwitchDebugOn()

   -- Init MANTIS
   RedMANTIS = MANTIS:New("RedMantis", "Red SAM", "Red EWR", "REDHQ", "red", false)
...
testMessage = MESSAGE:New( "Loaded TIW v0.0.3!", 30):ToAll()